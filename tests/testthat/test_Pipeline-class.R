...
describe("initialize", {
  ...
  it("allows valid tte", {
    ...
    expect_r6_class(pipeline, "Pipeline")
  })
})
...
describe("run_rg", {
  ...
  it("produces different results when using weighted cif", {
    ...
    expect_false(rg$results$rg == weighted_rg$results$rg)
  })
...
describe("run_default_rg", {
  it("produces same results as advanced interface", {
    ...
    rg_simple <- pipeline$run_default_rg(
      heritability1 = list(
        trait          = "SCZ",
        relatives_kind = "half_siblings",
        relatedness    = 0.25
  ...
  it("produces different results when using weighted cif", {
    ...
    weighted_rg <- pipeline$run_default_rg(
      heritability1 = list(
        trait          = "SCZ",
        relatives_kind = "half_siblings",
        relatedness    = 0.25
