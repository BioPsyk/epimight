Feature: getting TTE for multiple disorders

  Background:
    Given the following set of people
      | id |    born_at | gender | status          |
      |  1 | 1983-08-10 | female | danish-resident |
      |  2 | 1985-12-04 | male   | danish-resident |
      |  3 | 1987-01-30 | male   | danish-resident |
      |  4 | 1981-05-10 | female | danish-resident |
      |  5 | 1979-08-12 | male   | danish-resident |
      |  6 | 1983-10-03 | female | danish-resident |
    And the following set of peoples birthplaces
     | person_id | birthplace_name |
     |         1 | Gentofte Sogn   |
     |         2 | Valby Sogn      |
     |         3 | Brede Sogn      |
     |         4 | Gerlev Sogn     |
     |         5 | Sverige         |

  Scenario: all born in Denmark with two disorders
    Given person 1 was diagnosed with the following set of diagnoses
     | diagnosis_at | kind | icd_edition | icd_id |
     |   1990-05-23 | main | icd8        | 30650  |
     |   1996-01-02 | main | icd10       | F500   |
    And person 2 was diagnosed with the following set of diagnoses
     | diagnosis_at | kind | icd_edition | icd_id |
     |   1990-05-23 | main | icd8        | 30650  |
    And person 3 was diagnosed with the following set of diagnoses
     | diagnosis_at | kind | icd_edition | icd_id |
     |   1996-01-02 | main | icd10       | F500   |
    And person 4 was diagnosed with the following set of diagnoses
     | diagnosis_at | kind | icd_edition | icd_id |
     |   1990-05-23 | main | icd8        | 30650  |
    When I retrieve TTE for the following diagnoses
      | key   | icd_codes_regexp |
      | icd10 | ^F500            |
      | icd8  | ^30650           |
    Then the results contains the following rows
      | person_id | icd8_failure_status | icd8_failure_at | icd10_failure_status | icd10_failure_at |
      |         1 |                   1 |      1990-05-23 |                    1 |       1996-01-02 |
      |         2 |                   1 |      1990-05-23 |                    0 |       2016-12-31 |
      |         3 |                   0 |      2016-12-31 |                    1 |       1996-01-02 |
      |         4 |                   1 |      1990-05-23 |                    0 |       2016-12-31 |

  Scenario: all born in Denmark with three disorders
    Given person 1 was diagnosed with the following set of diagnoses
     | diagnosis_at | kind | icd_edition | icd_id |
     |   2000-01-01 | main | icd10       | F500   |
     |   2001-01-01 | main | icd10       | F502   |
     |   2002-01-01 | main | icd10       | F509   |
    And person 2 was diagnosed with the following set of diagnoses
     | diagnosis_at | kind | icd_edition | icd_id |
     |   2000-01-01 | main | icd10       | F500   |
     |   2001-01-01 | main | icd10       | F502   |
    And person 3 was diagnosed with the following set of diagnoses
     | diagnosis_at | kind | icd_edition | icd_id |
     |   1996-01-02 | main | icd10       | F500   |
    When I retrieve TTE for the following diagnoses
      | key | icd_codes_regexp |
      | ano | ^F500            |
      | bul | ^F502            |
      | uns | ^F509            |
    Then the results contains the following rows
      | person_id | ano_failure_status | ano_failure_at | bul_failure_status | bul_failure_at | uns_failure_status | uns_failure_at |
      |         1 |                  1 |     2000-01-01 |                  1 |     2001-01-01 |                  1 |     2002-01-01 |
      |         2 |                  1 |     2000-01-01 |                  1 |     2001-01-01 |                  0 |     2016-12-31 |
      |         3 |                  1 |     1996-01-02 |                  0 |     2016-12-31 |                  0 |     2016-12-31 |
      |         4 |                  0 |     2016-12-31 |                  0 |     2016-12-31 |                  0 |     2016-12-31 |
