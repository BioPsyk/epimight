...
c1_tte <- inner_join(d1_tte, d2_tte, by = join_by(person_id)) |>
  ...
  select(person_id, relatives, birth_year, starts_with("d1_"), starts_with("d2_"))
...
run_h2_stratified <- function(c1_estimates, c2_estimates) {
  combined_estimates <- c1_estimates |>
    inner_join(c2_estimates, by = join_by(time, birth_year)) |>
    rename(
    ...
    select(time, birth_year, starts_with("cohort"))
...
re_combined <- inner_join(re_d1_c1, re_d1_c3, by = join_by(time, birth_year)) |>
  rename(
  ...
  select(time, birth_year, starts_with("re_")) |>
  inner_join(re_d2_c1, by = join_by(time, birth_year)) |>
  rename(
...
h2_combined <- inner_join(h2_d1, h2_d2, by = join_by(time, birth_year)) |>
  rename(
    h2_d1 = h2.x,
    h2_d2 = h2.y
  ) |>
  select(time, birth_year, starts_with("h2_"))

# Join cumulative incidence and heritability estimates into a single dataset
combined <- inner_join(re_combined, h2_combined, by = join_by(time, birth_year)) |>
  group_by(birth_year) |>
  arrange(desc(time)) |>
  filter(row_number() == 1) |>
  arrange(birth_year) |>
...
print(gc_d1_d2 |> select(birth_year, rhh, rhog, se, l95, u95) |> arrange(birth_year))
