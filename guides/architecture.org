...
#+LATEX: \vspace{0.5cm}
#+LATEX: \begin{center}
#+NAME: fig:pipeline-domain-model
#+ATTR_HTML: :style max-width: 100%;
#+BEGIN_SRC plantuml :file ./diagrams/pipeline-domain-model.png :exports results
@startuml
...
actor "Sample" as sample
actor "Relatives" as relatives
collections "**Disorder 1**\nDiagnoses" as d1
collections "**Disorder 2**\nDiagnoses" as d2
Boundary1(exp_in, "Experiment input")
Boundary1(epimight, "Epimight pipeline")
Boundary1(exp_out, "Experiment output")
rectangle "Fixed model" as fixed_model
rectangle "Random model" as random_model
...
sample -- exp_inp
exp_inp -->
epimight ..> fixed_model
epimight ..> random_model
@enduml
#+END_SRC

#+ATTR_LATEX: :placement [H]
#+CAPTION: Pipeline domain model
#+RESULTS: fig:pipeline-domain-model
[[file:./diagrams/pipeline-domain-model.png]]
...
' rectangle "Experiment input" as exp_input {
'   actor "Sample" as samples
'   actor "Relatives" as relatives
'   file "**Disorder 1**\nDiagnoses" as d1
'   file "**Disorder 2**\nDiagnoses" as d2
'
'   samples -UP-> relatives
'   samples -UP-> d2
'   samples -UP-> d1
' }
...
file results [
<color:#255ba7><$ma_matrix{scale=0.35}></color>  **Experiment results**
